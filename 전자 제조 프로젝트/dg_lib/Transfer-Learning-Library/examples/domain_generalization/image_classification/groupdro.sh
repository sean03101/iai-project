...
#멀티소스
CUDA_VISIBLE_DEVICES=0 python groupdro.py data/Cognex -d Cognex -s Custom_repeat Custom_cameraz Custom_light Custom_bright Custom_blur -t Repeat Brightness Cameraz Lcondition -a resnet50 --log logs/groupdro/Cognex



CUDA_VISIBLE_DEVICES=0 python groupdro.py data/Cognex -d Cognex -s Custom_repeat Custom_contrast Custom_dropout Custom_flip Custom_noise Custom_cameraz Custom_light Custom_bright Custom_blur -t Repeat Brightness Cameraz Lcondition -a resnet50 --log logs/groupdro/Cognex  --lr 0.01


CUDA_VISIBLE_DEVICES=0 python groupdro.py data/Cognex -d Cognex -s Custom_repeat Custom_bright Custom_cameraz  -t Repeat Brightness Cameraz Lcondition -a resnet50 --log logs/groupdro/Cognex  --lr 0.01 --n-domains-per-batch 2
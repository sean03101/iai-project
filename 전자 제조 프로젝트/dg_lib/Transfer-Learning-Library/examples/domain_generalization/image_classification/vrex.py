...
def main(args: argparse.Namespace):
    ...
    for i in range(1):
        # Data loading code
        early_stopping = EarlyStopping(patience = 3, verbose = True, min_epoch=25)
        train_transform = utils.get_train_transform(args.train_resizing, random_horizontal_flip=False,
                                                    random_color_jitter=False, random_gray_scale=False)
        ...
        train_dataset, num_classes = utils.get_dataset(dataset_name=args.data, root=args.root, task_list=args.sources,
                                                    split='train', download=False, transform=train_transform,
                                                    seed=args.seed)
        ...
        val_dataset, _ = utils.get_dataset(dataset_name=args.data, root=args.root, task_list=args.sources, split='val',
                                        download=False, transform=val_transform, seed=args.seed)
        ...
        if args.phase == 'analysis':
            ...
            #utils.test_visualization(test_loader, classifier, args, device, save_path='/home/iai/Desktop/SH/transfer_library/Transfer-Learning-Library/examples/domain_generalization/image_classification/logs_single/vrex/Cognex_single/visualize/')
            return

        if args.phase == 'test':
            acc1 = utils.validate(test_loader, classifier, args, device)
            utils.test_visualization(test_loader, classifier, args, device, save_path=args.log + '/visualize/') 
            print(acc1)
            return
        ...
        for epoch in range(args.epochs):
            ...
            loss1 = utils.validate_loss(val_loader, classifier, args, device) 
            early_stopping(loss1, classifier)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-resizing', type=str, default='Cognex')
    parser.add_argument('--val-resizing', type=str, default='Cognex')
    # model parameters
    ...
    parser.add_argument('--anneal-iters', default=138, type=int,
                        help='anneal iterations (trade off is set to 1 during these iterations)')
    parser.add_argument('-b', '--batch-size', default=32, type=int,
                        metavar='N',
                        help='mini-batch size (default: 36)')
    parser.add_argument('--n-domains-per-batch', default=4, type=int,
    ...
    parser.add_argument('-i', '--iters-per-epoch', default=138, type=int,
                        help='Number of iterations per epoch')
    parser.add_argument('-p', '--print-freq', default=46, type=int,

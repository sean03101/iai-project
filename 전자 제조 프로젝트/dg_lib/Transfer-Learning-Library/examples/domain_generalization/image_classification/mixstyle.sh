...
#멀티소스
CUDA_VISIBLE_DEVICES=0 python mixstyle.py data/Cognex -d Cognex -s Custom_repeat Custom_cameraz Custom_light Custom_bright Custom_blur -t Repeat Brightness Cameraz Lcondition -a resnet50 --mix-layers layer1 layer2 --seed 0 --log logs/mixstyle/Cognex

CUDA_VISIBLE_DEVICES=0 python mixstyle.py data/Cognex -d Cognex -s Custom_repeat Custom_bright Custom_cameraz -t Repeat Brightness Cameraz Lcondition -a resnet50 --mix-layers layer1 layer2 layer3 --log logs/mixstyle/Cognex  --lr 0.01
